...
- Users table with Replit Auth fields (id, name, email, image, **airtableUserId**)
- Sessions table for authentication state management
- Drizzle migrations system with PostgreSQL dialect

**User ID Synchronization**: Dual-database user linking
- **PostgreSQL User ID**: Generated by Replit Auth (UUID format) for authentication
- **AirTable User ID**: AirTable record ID (e.g., "recXXXXXXXXXXXXXX") for application data
- **Linking Strategy**: `airtableUserId` column in PostgreSQL users table stores AirTable record ID
- **Automatic Sync**: During login, `replitAuth.ts` automatically finds/creates AirTable user and saves the AirTable ID
- **Router Access**: All tRPC endpoints use `ctx.user.airtableUserId` to query portfolios and application data
- **Backfill Support**: Existing users can be linked via `scripts/backfill-airtable-users.ts` (matches by email)
...
- User authentication data stored in PostgreSQL with AirTable user ID link
- AirTable user created/updated automatically during login via `replitAuth.upsertUser()`
